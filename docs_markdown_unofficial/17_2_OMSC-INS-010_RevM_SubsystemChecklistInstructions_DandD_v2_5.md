...
OMS Subsystem Checklist Instructions

<img src="images/17_2_OMSC-INS-010_RevM_SubsystemChecklistInstructions_DandD_v2_5.docx/media/image1.jpeg" style="width:2.78125in;height:1.73958in" />
...
The Subsystem Checklist is an Excel workbook used to assist the
Subsystem provider in creating the compliance documentation for an OMS
Subsystem. The Checklist uses the Subsystem Description Document (SDD)
and Service Contract for the Subsystem.

This OMS Subsystem Checklist supports the compliance process per the OMS
...
[1 Subsystem Checklist 1](#subsystem-checklist)
...
[1.2.6 “Subsystem RQMT Checklist” Tab 2](#subsystem-rqmt-checklist-tab)

[1.2.7 “SDD” Tab 2](#sdd-tab)

[1.2.8 “Service Contract” Tab 3](#service-contract-tab)

[1.2.9 “OMS Msgs” Tab 4](#oms-msgs-tab)

[1.2.10 “Non-OMS Msgs” Tab 7](#non-oms-msgs-tab)

[1.2.11 “OMS Data Transfer” Tab 7](#oms-data-transfer-tab)

[1.2.12 “Non-OMS Data Transfer” Tab 9](#non-oms-data-transfer-tab)

[1.2.13 “OMS Special Signals” Tab 10](#oms-special-signals-tab)

[1.2.14 “Non-OMS Special Signals” Tab 10](#non-oms-special-signals-tab)

[1.2.14.1 Section A. Non-OMS Special Signals Allowed in the OMS Standard
11](#section-a.-non-oms-special-signals-allowed-in-the-oms-standard)

[1.2.14.2 Section B. Non-OMS Special Signals Superseded by OMS Messages
11](#section-b.-non-oms-special-signals-superseded-by-oms-messages)

[1.2.14.3 Section C. Other Non-OMS Special Signals
12](#section-c.-other-non-oms-special-signals)

[1.2.15 “Data Transfer Tables” and “Special Signals Tables” Tabs
12](#data-transfer-tables-and-special-signals-tables-tabs)

[2 References 13](#references)
...
Checklist Tabs 5](#_Toc219357465)

This page is intentionally left blank.

List of Tables

[Table 2.0-1 OMS D&D Documents 13](#_Toc219357466)
...
The OMS Subsystem Checklist is an Excel workbook with a file name of
17\_1\_OMSC-CHK-004\_RevM\_SubsystemChecklist\_DandD\_v2\_5.xlsx

The Checklist is used by the Subsystem provider to generate the required
compliance documentation for an OMS Subsystem. The Checklist uses the
Subsystem Description Document (SDD) and Service Contract, which may
include separate documents referenced in the Security Addendums. Before
attempting to fill out the checklist, the Subsystem compliance assessor
should have access to the referenced SDD and Service Contract for the
Subsystem.

In the Checklist, all green text must be replaced with the specific
information for the Subsystem being documented and the workbook should
be saved using an appropriate filename.
...
the information for the Subsystem.

User input is required on the ‘Blue’ tabs highlighted by the blue
shaded, which are user editable.
...
The OMS Subsystem Checklist (workbook) includes the following tabs:
...
locked and contain formulas that cross-reference the Subsystem RQMT
Checklist and perform an automated calculation of the Tier Compliance.
There are no user entries required on this tab.

### “Subsystem RQMT Checklist” Tab

This tab lists each Requirement (RQMT) allocated to an OMS Subsystem in
...
This tab is comprised of the outline for a Subsystem Description
Document (SDD) per the SDD Template, OMSC-TMP-002. Each item of the
...
template item; indicating that the template item is included in the SDD
for the OMS Subsystem being documented. An indication of TRUE requires
...
-   Deleted from or does not exist in the SDD:
...
<img src="images/17_2_OMSC-INS-010_RevM_SubsystemChecklistInstructions_DandD_v2_5.docx/media/image2.png" style="width:6.5in;height:2.76944in" alt="Diagram AI-generated content may be incorrect." />

<span id="_Toc219357465" class="anchor"></span>Figure 1.2-1 Mapping of
...
-   In column S, enter the location (multiple locations can be listed)
    ...
    passed through a CAL between the Subsystem and a test harness.
...
EXAMPLES: A Subsystem provider can either initiate a message
modification or may be requested to support a message modified by
another Architecture Element provider. A modification or extension that
extends messages in either of the MMSs may temporarily cause an
Architecture Element using these messages not be OMS compliant at the
intended Tier for the specific OMS release version until the change is
approved by the OACWG. Note that asserting or proving compliance will
change as a program evolves from a proposal/acquisition phase (e.g.,
initial compliance assessments) to a program after final testing (e.g.,
...
modification is approved by the OACWG

-   Tier 2 Example: A Subsystem modifies the **MissionPlan** Tier 2 MMS
    message

Modification to an OMS Tier 2 MMS makes the Subsystem not compliant at
OMS Tier 2 until the modification is approved by the OACWG. If all other
Tier 1 compliance requirements are met, Tier 1 compliance could be
achieved

-   Tier 3 Example: A Subsystem modifies the
...
1.2.9 “OMS Msgs” Tab. Non-OMS Messages are not compliant at any tier.

For this tab, the user completes the shaded columns for any Non-OMS
Messages used by the Subsystem. If no Non-OMS Messages are used by the
Subsystem then all the shaded fields should be left at their default
...
be entered. If a Non-OMS Message is used, then the Subsystem is not OMS
...
by the Subsystem by filling in each shaded cell. Shaded cells left blank
...
-   Column G: Select the URI format from the pull-down list that matches
...
used by the Subsystem by filling in each shaded cell. Shaded cells left
...
-   Column G: 1) Select the URI format from the pull-down list that
...
Signal used by the Subsystem. The tables in this tab are drawn from the
tables of the same name in the OMS Standard. The user simply completes a
row for each OMS Special Signal used by the Subsystem by selecting the
...
any other non-OMS special signals used by the Subsystem that are not
...
If the Service Contract documents that the Subsystem uses a non-OMS
...
If the Service Contract documents that the Subsystem uses other non-OMS
special signals that are not listed in the “Special Signals Superseded
by OMS Messages” table, the user will identify those non-OMS special
signals in Section B (middle section) of the \[\[Non-OMS Special
...
<span id="_Toc219357466" class="anchor"></span>Table 2.0-1 OMS D&D
Documents
