...
The Isolator Checklist is an Excel workbook used to assist the Isolator
provider in creating the compliance documentation for an OMS Isolator.
The Checklist uses the Service Contract for the Isolator.

This OMS Isolator Checklist supports the compliance process per the OMS
...
[1 Isolator Checklist 1](#isolator-checklist)
...
[1.2.6 “Isolator RQMT Checklist” Tab 2](#isolator-rqmt-checklist-tab)

[1.2.7 “Service Contract” Tab 2](#service-contract-tab)

[1.2.8 “OMS Msgs” Tab 4](#oms-msgs-tab)

[1.2.9 “Non-OMS Msgs” Tab 6](#non-oms-msgs-tab)

[1.2.10 “OMS Data Transfer” Tab 6](#oms-data-transfer-tab)
...
9](#section-a.-non-oms-special-signals-allowed-in-the-oms-standard)
...
10](#data-transfer-tables-and-special-signals-tables-tabs)
...
Checklist Tabs 4](#_Toc219358755)

This page is intentionally left blank.

List of Tables

[Table 2.0-1 OMS D&D Documents 11](#_Toc219358756)
...
The OMS Isolation Checklist is an Excel workbook with a file name of
19\_1\_OMSC-CHK-006\_RevM\_IsolatorChecklist\_DandD\_v2\_5.xlsx

The Checklist is used by the Isolator provider to generate the required
compliance documentation for an OMS Isolator. The Checklist uses the
...
Isolator compliance assessor should have access to the referenced
Service Contract for the Isolator.

In the Checklist, all green text must be replaced with the specific
information for the Isolator being documented and the workbook should be
...
the information for the Isolator.
...
The OMS Isolator Checklist (workbook) includes the following tabs:
...
This tab contains the Tier Compliance Summary. The OMS Isolator is not
evaluated directly for “Tier X” compliance. Instead, an OMS Isolator is
evaluated on whether or not it meets the requirements for use in a “Tier
X” OMS Mission Package. The cells of this tab are locked and contain
formulas that cross-reference the Isolator RQMT Checklist and perform an
automated calculation. The result is posted at the top of the tab and is
expressed as one of four possibilities:

1.  “Does Not Meet Isolator RQMTs for an OMS Mission Package”

2.  “Meets Isolator RQMTs for a Tier 1 OMS Mission Package”

3.  “Meets Isolator RQMTs for a Tier 1 or Tier 2 OMS Mission Package”

4.  “Meets Isolator RQMTs for a Tier 1, Tier 2, or Tier 3 OMS Mission
    Package.”

There are no user entries required on this tab.

### “Isolator RQMT Checklist” Tab

This tab lists each Requirement (RQMT) allocated to an OMS Isolator in
...
for the OMS Isolator being documented. An indication of TRUE requires
...
An Isolator is a passive Architecture Element that may need to consume
Minimum Message Set (MMS) messages that have been modified by other
Architecture Elements. Isolators do not have individually-assessed
compliance tiering. An Isolator provider typically does not initiate a
message modification, but may be requested to support a message modified
by another Architecture Element provider. This may affect achieving OMS
compliance at an intended Mission Package tier level. Even though an
Isolator is not tiered, its Service Contract must reflect these modified
messages. At Tier 3, all OMS Messages used in an OMS Message Set must
have been approved by the Open Architecture Collaborative Working Group
(OACWG) as defined in the OACWG Governance Plan (i.e., they are Approved
OMS Messages).
...
-   In column F “Used”, use pull-down list to indicate if the message is
    used:
...
-   In column S, enter the location (multiple locations can be listed)
    ...
    passed through a CAL between the Isolator and a test harness.
...
Messages used by the Isolator. If no Non-OMS Messages are used by the
Isolator then all the shaded fields should be left at their default
...
by the Isolator by filling in each shaded cells. Shaded cells left blank
...
For this tab, the user completes a row for each Non-OMS Data Transfers
used by the Isolator by filling in each shaded cells. Shaded cells left
...
Signal used by the Isolator. The tables in this tab are drawn from the
tables of the same name in the OMS Standard. The user simply completes a
row for each OMS Special Signal used by the Isolator by selecting the
...
any other non-OMS special signals used by the Isolator that are not
...
If the Service Contract documents that the Isolator uses a non-OMS
...
If the Service Contract documents that the Isolator uses other non-OMS
...
<span id="_Toc219358756" class="anchor"></span>Table 2.0-1 OMS D&D
Documents
